...
contract TipJar {
    ...
    function addCurrency(string memory _currencyCode, uint256 _rateToEth) public onlyOwner {
        require(_rateToEth > 0, "Conversion rate must be greater than 0");//检查比率是否为零或负——只是快速的安全检查。
        bool currencyExists = false;
        for (uint i = 0; i < supportedCurrencies.length; i++) { //不能像在 JavaScript 或 Python 中那样直接使用 == 比较两个字符串。这是因为 Solidity 中的字符串是存储在内存中的复杂类型，而不是原始值。
