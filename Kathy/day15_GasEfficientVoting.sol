...
contract GasEfficientVoting {
    ...
    struct Proposal {
        string name; // Use bytes32 instead of string to save gas
        uint32 voteCount; // Use uint32 instead of uint256
        uint32 startTime;
        uint32 endTime;
        bool executed;
    }
    ...
    // using a mapping instead of embedding mapping
    ...
    function exectueProposal(uint8 proposalId) external {
        require(proposalId < proposalCount, "Invalid proposal");
     ...
     function getPorposal(uint8 proposalId) external view returns (
        bytes32 name,
        uint32 voteCount,
        uint32 startTime,
        uint32 endTime, 
